...
census_api_key(Sys.getenv("dbc38b2c26ce30078c0e20a83c42a42dd8ea8610"))
...
**Pattern Recognition: 001 is All, 002 is Male, and 003 is Female \| A,
B, C, D is by Race\
Answer:** Median age is typically B01002_001
...
ggplot(county_data) +
  ...
  labs(
    title = "Histogram of Total Population by County",
    x = "Total Population",
    y = "Number of Counties",
    caption = "Source: U.S. Census Bureau 2018-2022 ACS"
  )

# Boxplot
ggplot(county_data) +
  aes(y = total_popE) +
  geom_boxplot(fill = "lightgreen", width = 0.1) +
  labs(
    title = "Population Distribution with Outliers",
    y = "Total Population"
  ) +
  theme_minimal() +
...
pop_outliers_highest <- pop_outliers |> 
  arrange(desc(total_popE))
slice(pop_outliers_highest, 1:3)

pop_outliers_lowest <- pop_outliers |> 
  arrange(total_popE)
slice(pop_outliers_lowest, 1:3)
```
...
ggplot(county_data) +
  ...
  labs(
    title = "Relationship between Median Age and Median Income",
    x = "Median Age",
    y = "Median Income"
  ) +
  theme_dark() +
...
-   Add a horizontal line at 5% MOE using `geom_hline()` with a blue
    dashed line
-   Use `theme_classic()`instead of `theme_minimal()`
-   Create a boxplot comparing age MOE across the three population
    categories

```{r}
county_reliability <- county_reliability |> 
  mutate(age_moe_pct = median_ageM / median_ageE * 100)
...
ggplot(county_reliability) +
  ...
  labs(
    title = "Age Data Reliability Decreases with Population Size",
    x = "Total Population",
    y = "Margin of Error (%)",
...
ggplot(county_reliability) +
  ...
  labs(
    title = "Age Data Reliability by County Size Category",
    x = "Population Category",
    y = "Margin of Error (%)"
  ) +
  theme_classic() +
...
**Your Task:** Create a visualization using income categories and
multiple aesthetic mappings.

**Requirements:**

-   Create income categories: "Low" (\<\$50k), "Middle" (\$50k-\$80k),
    "High" (\>\$80k)
-   Make a scatter plot with population (x) vs median age (y) - Color
    points by income category
-   Size points by the margin of error for income (`median_incomeM`)
-   Use the "Set2" color palette: `scale_color_brewer(palette = "Set2")`
    \*\*note: you'll need to load the `RColorBrewer` package for this\`
...
**Your Task:** Combine multiple visualizations to tell a more complete
story about county characteristics.
