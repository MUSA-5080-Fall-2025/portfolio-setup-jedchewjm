...
phl_poverty_sch_scatter <- ggplot(phl_summary_clean,
  ...
  theme_minimal(base_size = 12)
phl_poverty_sch_scatter
```

As shown in the scatter plots above, there is no single clear relationship
between the distance to the nearest school and the percentage of children or the
level of poverty in a census tract. However, we can still do some explanatory
analysis to further classify schools by their average attendance levels.
...
head(attendance_clean)
```

```{r}
# Spatial Join Attendance to School Locations
...
1.  Hide the setup code block. The Census API key is not supposed to be shared
    publicly. I used "include = FALSE" and "results = 'hide'" in the relevant R
    code chunks
